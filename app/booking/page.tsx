  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Button
        type="submit"
        disabled={
          !stripe || isProcessing || !termsAccepted || !cancellationAccepted
        }
...
export default function BookingPage() {
    ...
    },
    {
      ...
      pricing: {
        inPerson: {
          single: 150,
          packages: [
            { id: "single", sessions: 1, price: 150, name: "Single Session" },
          ],
        },
        distance: {
          single: 120,
  ...
  // Helper function to convert Dubai time to user's local time
  const convertDubaiTimeToLocal = (perthTimeString: string, date: Date) => {
    // Parse the Dubai time slot (e.g., "08:00-09:30")
    const [startTime, endTime] = perthTimeString.split("-");
    const [startHour, startMin] = startTime.split(":").map(Number);
    const [endHour, endMin] = endTime.split(":").map(Number);

    // Create a date string in Dubai timezone format
    const year = date.getFullYear();
    const month = String(date.getMonth() + 1).padStart(2, "0");
    const day = String(date.getDate()).padStart(2, "0");

    // Create Dubai datetime strings
    const perthStartString = `${year}-${month}-${day}T${startTime}:00`;
    const perthEndString = `${year}-${month}-${day}T${endTime}:00`;

    // Convert Dubai times to user's local timezone using proper timezone handling
    const convertToLocal = (perthTimeString: string) => {
      // Create a date object representing the time in Dubai
      const perthDate = new Date(perthTimeString);

      // Create a temporary date to handle Dubai timezone
      ...
      const localDate = new Date(
        tempDate.toLocaleString("sv-SE", { timeZone: "Australia/Dubai" }),
      );
    ...
    const formatLocalTime = (hour: number, minute: number) => {
      // Create a date object representing Dubai time
      ...
      // Convert Dubai time to local time
      // Dubai is UTC+8, so we subtract 8 hours to get UTC, then convert to local
  ...
  // Helper function to get Dubai UTC offset (handles daylight saving)
  const getDubaiUTCOffset = (date: Date): number => {
    // Create a date in Dubai timezone to check if DST is active
    const perthDate = new Date(
      date.toLocaleString("en-US", { timeZone: "Australia/Dubai" }),
    );
  ...
  const generateTimeSlots = (date: Date, type: string) => {
    // Base Dubai time slots (these remain as your source of truth)
    ...
    // Convert each Dubai time slot to user's local time
    const localTimeSlots = perthTimeSlots.map((perthSlot) =>
      convertDubaiTimeToLocal(perthSlot, date),
    );
  ...
  const selectedProgramData = selectedServiceData?.specialPrograms?.find(
  ...
  const canProceedToStep2 =
    selectedService && selectedDate && selectedTime && sessionType;
  ...
  const getCurrentPrice = () => {
    ...
    if (selectedServiceData?.pricing) {
      ...
      if (pricingSection && selectedPackage) {
        const packageData = pricingSection.packages?.find(
    ...
    if (selectedServiceData?.packages && selectedPackage) {
      const packageData = selectedServiceData.packages.find(
        (pkg: any) => pkg.id === selectedPackage,
      );
      if (packageData) {
        return packageData.price;
  ...
  const getCurrentTitle = () => {
    ...
    if (selectedPackage && selectedServiceData) {
      ...
      if (selectedServiceData.pricing) {
        ...
        packageData = pricingSection?.packages?.find(
          (pkg: any) => pkg.id === selectedPackage,
        );
      } else if (selectedServiceData.packages) {
        packageData = selectedServiceData.packages.find(
          (pkg: any) => pkg.id === selectedPackage,
        );
      }
          ...
          <Card className="border-green-200">
            <CardContent className="p-8 text-center space-y-6">
              ...
              <p className="text-stone-600">
                Thank you for booking your healing session. You will receive a
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p>
                      ...
                      {bookingConfirmation.paymentIntentId}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <ul className="text-sm text-stone-600 space-y-2 text-left">
                  ...
                  <li>
                    • Prepare any questions you'd like to discuss during your
                    session
                  </li>
                </ul>
              </div>
      ...
      <section className="py-12 bg-gradient-to-br from-stone-100 to-sage-50">
        <div className="container mx-auto px-4">
          <div className="text-center space-y-4">
            ...
            <h1 className="text-3xl lg:text-4xl font-serif text-stone-800">
              Schedule Your Healing Session
            </h1>
            <p className="text-stone-600 max-w-2xl mx-auto">
              Book your personalized healing session in just a few simple steps.
              ...
              <div className="space-y-8">
                <Card>
                  ...
                  <CardContent className="space-y-8">
                                    ...
                                    >
                                      <CardContent className="p-3 space-y-2">
                                        <div className="flex justify-between items-start">
                                          <div className="flex-1">
                                            <h6 className="font-medium text-stone-800 text-sm">
                                              {program.name}
                                            </h6>
                              ...
                              >
                                <CardContent className="p-4 space-y-2">
                                  ...
                                  <p className="text-sm text-stone-600">
                                    Visit our peaceful practice in Dubai
                                  </p>
                                  <p className="text-xs text-stone-500">
                                    Dubai Healthcare Centre, Ground floor, Shop
                                    2/1260, Hay Street, West Dubai WA 6005,
                                    Australia
                                  </p>
                            ...
                            {selectedServiceData.pricing.distance && (
                              ...
                              >
                                <CardContent className="p-4 space-y-2">
                                  ...
                                  <p className="text-sm text-stone-600">
                                    Secure video call from your home
                                  </p>
                                  <p className="text-xs text-stone-500">
                    ...
                    (selectedService && sessionType && selectedPackage) ? (
                    ...
                    {/* Summary */}
                    {(selectedServiceData || selectedProgramData) &&
                      selectedDate &&
                      selectedTime && (
              ...
              <div className="space-y-8">
                <Card>
                  <CardHeader>
                    ...
                    <p className="text-sm text-stone-600">
                      This information helps us provide the best possible care.
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                      ...
                      <p className="text-sm text-amber-700">
                        Our healing services are complementary and not a
                        substitute for medical treatment. If you have serious
                        mental health concerns, please consult with your
                        healthcare provider. We may recommend you seek medical
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <div className="bg-stone-50 p-6 rounded-lg space-y-4">
                      ...
                      <div className="grid md:grid-cols-2 gap-4 text-sm">
                        <div className="space-y-2">
                          <p>
                            <strong>Client:</strong> {bookingData.firstName}{" "}
                            {bookingData.lastName}
                          </p>
                          <p>
